...
// get all hospital list
  const getHospitalRecord = async () => {
    try {
      const response = await fetch('/api/inventory/hospital-list');
      if (!response.ok) {
      ...
      setRecordedData(data.hospitalData);
      setIsLoading(false);
    } catch (error) {
  ...
  useEffect(() => {
   ...
   // delete specific hospital from hospital list using id
...
export default HospitalListContent
